...
struct PtSort {
bool operator()(TLorentzVector a, TLorentzVector b){ return a.Pt() > b.Pt(); };
};
...
void rdphi::SlaveBegin(TTree * /*tree*/)
{
  ...
  //############### Load JES uncertainties

  if(do_jes) {
     myJES = new MultijetJESUncertaintyProvider( "MultijetJES_2012.config",
                                                 "InsituJES2012_AllNuisanceParameters.config",
                                                 "AntiKt6TopoLC",
                                                 "MC12a", "/user/rsu004/2014/rdphi/JES/");  
  }
/*
  //alternate JES 
  if(do_jes) {
     myJES = new MultijetJESUncertaintyProvider( "MultijetJES_2012.config",
                                                 "InsituJES2012_StrongerCorrelations.config",
                                                 "AntiKt6TopoLC",
                                                 "MC12a", "/user/rsu004/2014/rdphi/JES/");  
  }
...
*/
  ...
  //initialize TileTripreader
 // m_treader->setTripFile("TileTripReader/data/CompleteTripList_2011-2012.root");
...
Bool_t rdphi::Process(Long64_t entry)
{
   ...
   //event level phase space, trigger checks 
   ...
   if ( jet_pt->size() >=2 )
       {
           if (  jet_Badness->at(1) >=3  || jet_Badness->at(0) >=3      )  return kFALSE;
           if (  jet_pt->at(1)/1000. < 100. ) return kFALSE;          
           ystar  = 0.5 *    fabs( jet_y->at(0) - jet_y->at(1) ); 
           yboost = 0.5 *        ( jet_y->at(0) + jet_y->at(1) ); 
           if ( ystar > 2) return kFALSE;
           if ( fabs(yboost) > 0.5) return kFALSE;                           
...
//debo
                     if (  (jet_pt->at(0)/1000) < (ht/3) )        return kFALSE;               
  ...
  // if (reject_event || !trigger_passed)  rejected_events++;
   ...
   if ( jet_pt->size() >=2 && trigger_passed)
      {
       HT->Fill(ht);
